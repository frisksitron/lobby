...
Client → Server commands: `IDENTIFY`, `RESUME`, `MESSAGE_SEND`, `TYPING`, `VOICE_JOIN`, `VOICE_LEAVE`, `VOICE_STATE_SET`, `RTC_OFFER`, `RTC_ANSWER`, `RTC_ICE_CANDIDATE`

Server → Client events: `PRESENCE_UPDATE`, `MESSAGE_CREATE`, `MESSAGE_ACK`, `TYPING_START`, `TYPING_STOP`, `USER_UPDATE`, `VOICE_STATE_UPDATE`, `RTC_*`, `VOICE_SPEAKING`, `USER_JOINED`, `USER_LEFT`, `ERROR`
